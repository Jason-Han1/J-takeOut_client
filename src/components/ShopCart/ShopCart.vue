      ...
      <div class="content">
        <div class="content-left" @click="toggleShow">
          <div class="logo-wrapper">
            <div class="logo" :class="{hightlight: totalCount}">
              <i class="iconfont icon-shopping_cart highlight" :class="{highlight: totalCount}"></i>
            </div>
            <div class="num" v-if="totalCount">{{totalCount}}</div>
          </div>
          <div class="price" :class="{hightlight: totalCount}">￥{{totalPrice}}</div>
    ...
    computed: {
      ...
      listShow() {
        ...
        if(this.isShow){
          this.$nextTick(() => {
            if(!this.scroll){
              //必须实现单例对象因为click不是原生的，是scroll控制的，
              // 不实现单例点击一次可能count会加很多次
